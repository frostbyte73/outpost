...
export interface DiffFile {
  ...
  truncated: boolean;
  hunks: DiffHunk[];
  // `git diff HEAD` excludes untracked files. The diff endpoint synthesizes
  // entries for them in worktree mode (via `git diff --no-index`) so the PWA can
  // review their contents and stage them with the same checkbox UI. Content-bearing
  // entries carry hunks like any added file; only empty/over-budget ones are hunkless.
  untracked?: boolean;
}
