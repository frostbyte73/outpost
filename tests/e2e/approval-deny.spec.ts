...
test('Deny resolves the approval; mock claude emits deny tool_result instead of "done"', async ({ daemon, outpostPage }) => {
  ...
  // Send the rejection with the typed note. Desktop's card (session-view/approval-card.js)
  // names this class differently than the mobile-only approvals-mobile.js singleton
  // (.approval-reject-send vs .reject-send) — Playwright's default viewport is desktop-sized.
  await outpostPage.locator('button.approval-reject-send').first().click();
