...
describe('loadConfig', () => {
  beforeEach(() => {
    ...
    delete process.env.OUTPOST_HTTP_PORT;
  ...
  it('httpPort defaults to 8080; honors OUTPOST_HTTP_PORT', () => {
    expect(loadConfig().httpPort).toBe(8080);
    process.env.OUTPOST_HTTP_PORT = '8090';
    expect(loadConfig().httpPort).toBe(8090);
  });

  it('httpPort returns null when OUTPOST_HTTP_PORT=0 (disable sentinel)', () => {
    process.env.OUTPOST_HTTP_PORT = '0';
    expect(loadConfig().httpPort).toBeNull();
  });

  it('rejects invalid OUTPOST_HTTP_PORT values', () => {
    process.env.OUTPOST_HTTP_PORT = 'not-a-port';
    expect(() => loadConfig()).toThrow(/OUTPOST_HTTP_PORT/);
    process.env.OUTPOST_HTTP_PORT = '-1';
    expect(() => loadConfig()).toThrow(/OUTPOST_HTTP_PORT/);
    process.env.OUTPOST_HTTP_PORT = '70000';
    expect(() => loadConfig()).toThrow(/OUTPOST_HTTP_PORT/);
  });
});
